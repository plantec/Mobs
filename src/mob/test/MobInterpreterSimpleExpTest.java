...
class MobInterpreterSimpleExpTest {
	...
	void test() throws IOException {
		...
		assertTrue(res.get(0) instanceof MobObject);
		assertTrue((Integer)((MobObject) res.get(0)).primValue() == 10);
		...
		assertTrue(((MobObject)res.get(0)).primValue().equals((float)10.0));
		
		res = interpreter.run("-10.5");
		assertTrue(((MobObject)res.get(0)).isKindOf(env.getClassByName("Float")));
		assertTrue(((MobObject)res.get(0)).primValue().equals((float)-10.5));
		...
		assertTrue( ((MobObject) res.get(0)).primValue().equals('a'));

		res = interpreter.run("$$");
		assertTrue(((MobObject)res.get(0)).isKindOf(env.getClassByName("Character")));
		assertTrue( ((MobObject) res.get(0)).primValue().equals('$'));
		
		res = interpreter.run("'Hello world'");
		assertTrue(((MobObject)res.get(0)).isKindOf(env.getClassByName("String")));
		assertTrue( ((MobObject) res.get(0)).primValue().equals("Hello world"));
		...
		assertTrue( (((Boolean)((MobObject) res.get(0)).primValue())));
		
		res = interpreter.run("false");
		assertTrue(((MobObject)res.get(0)).isKindOf(env.getClassByName("False")));
		assertFalse( (((Boolean)((MobObject) res.get(0)).primValue())));
